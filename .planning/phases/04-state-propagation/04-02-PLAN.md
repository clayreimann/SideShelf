...
must_haves:
  truths:
    ...
    - "updateNowPlayingMetadata calls in BGS are removed (coordinator bridge handles metadata on chapter change)"
  artifacts:
...
2. `handlePlaybackProgressUpdated()`:
   ...
   - Remove `store.updateNowPlayingMetadata()` calls (~line 475 chapter change, ~line 483 periodic) — coordinator's syncStateToStore() now handles metadata on chapter change via the lastSyncedChapterId tracking. For periodic metadata updates (the `% 2 === 0` gate): the coordinator's syncPositionToStore does NOT call updateNowPlayingMetadata (it's position-only). **Resolution:** Keep the periodic `store.updateNowPlayingMetadata()` call gated by `periodicUpdatesEnabled && Math.floor(event.position) % 2 === 0`. This is a settings-gated behavior that the coordinator bridge doesn't replicate. Remove ONLY the chapter-change metadata call (coordinator bridge handles that).
...
- 1 write of store.updateNowPlayingMetadata (chapter-change path only; keep periodic path)
...
- `store.updateNowPlayingMetadata()` periodic path (settings-gated, `% 2 === 0`) — coordinator bridge doesn't replicate periodic metadata; only chapter-change metadata
  </action>
  <verify>
  Run: `npx tsc --noEmit 2>&1 | head -30` — no new TypeScript errors.
  ...
  <done>
  PlayerBackgroundService has ~17 direct store writes removed. Position, isPlaying, and isLoadingTrack are now written exclusively by the coordinator bridge. Retained writes are limited to lastPauseTime (ephemeral), cancelSleepTimer (PROP-04), and periodic metadata updates (settings-gated).
  </done>
  </task>
