...
export async function getAllActiveSessionsForUser(
  ...
  const sessions = await db
    ...
    .from(localListeningSessions)
    .where(
      and(eq(localListeningSessions.userId, userId), isNull(localListeningSessions.sessionEnd))
    );
 ...
 */
export async function recordSyncFailure(sessionId: string, error: string): Promise<void> {
  const now = new Date();

  const attempts = await getListeningSession(sessionId).then(
    (session) => session?.syncAttempts || 0
  );
  await db
    .update(localListeningSessions)
    .set({
...
export async function cleanupOldSnapshots(
  ...
  await db.delete(localProgressSnapshots).where(
    and(
...
export async function getListeningStats(
  ...
  const averageSessionLength =
    completedSessions.length > 0 ? totalListeningTime / completedSessions.length : 0;
