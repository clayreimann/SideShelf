...
export interface StorageEntry {
  ...
  count: number;
  size: number;
  /** Whether this item is excluded from iCloud backup (iOS only)
   * true = all excluded, false = none excluded, 'mixed' = some excluded, undefined = unknown
   */
  excludedFromBackup?: boolean | "mixed";
}
...
export const createStatisticsSlice: SliceCreator<StatisticsSlice> = (set, get) => ({
  ...
  refreshStatistics: async () => {
    ...
    try {
      ...
      log.info(
        `Statistics refreshed successfully: authors=${counts.authors}, genres=${counts.genres}, languages=${counts.languages}, narrators=${counts.narrators}, series=${counts.series}, tags=${counts.tags}`
      );
    } catch (error) {
