...
import { GestureResponderEvent, LayoutChangeEvent, PanResponder, Text, TextStyle, View, ViewStyle } from 'react-native';
...
export default function ProgressBar({
    ...
    const currentValue = seekValue > 0 ? seekValue : (currentTime !== undefined ? currentTime : progress * seekMaxValue);

    // For time display, show relative time within the range (seekValue - minValue) when seeking
    const displayTime = seekValue > 0 ? (seekValue - minValue) : (currentTime !== undefined ? currentTime : progress * seekMaxValue);

    // Calculate progress as a percentage (0-1) for display
    const progressPercentage = seekValue > 0
    ...
    const panResponder = PanResponder.create({
        ...
        onPanResponderGrant: (evt: GestureResponderEvent) => {
            ...
            const seekValue = Math.max(minValue, Math.min(seekMaxValue, minValue + (locationX / progressBarWidth) * seekRange));
