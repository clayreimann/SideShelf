...
export const createSeriesSlice: SliceCreator<SeriesSlice> = (set, get) => ({
  ...
  initializeSeries: async (apiConfigured: boolean, dbInitialized: boolean) => {
    ...
    try {
      ...
      // Set ready state (series only needs DB, not API)
      get()._setSeriesReady(apiConfigured, dbInitialized);

      // If DB is ready, fetch initial data
      // Note: Series data comes from local DB only, doesn't require API
      if (dbInitialized) {
        console.log("[SeriesSlice] DB ready, fetching initial data...");
