...
// More-stack authors list screen. Navigation paths are scoped to /more/authors/
// so that tapping an author opens the detail within the More stack (not the Authors tab stack).
...
export default function MoreAuthorsScreen() {
  ...
  const renderAuthor = React.useCallback(
    ({ item }: { item: any }) => {
      ...
      return (
        <TouchableOpacity
          onPress={() => router.push(`/more/authors/${item.id}`)}
          style={{
            flexDirection: "row",
            padding: 16,
            borderBottomWidth: 1,
