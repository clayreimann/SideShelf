...
export interface CachedItemDetails {
  ...
  /** First author's authorId for navigation, null if none */
  authorId: string | null;
  /** First seriesId for navigation, null if none */
 ...
 */
export interface LibraryItemDetailsSlice
  extends LibraryItemDetailsSliceState,
    LibraryItemDetailsSliceActions {}
...
export const createLibraryItemDetailsSlice: SliceCreator<LibraryItemDetailsSlice> = (set, get) => ({
  ...
  clearOldCache: () => {
    ...
    log.info(
      `Cache size ${cacheSize} exceeds max ${state.itemDetails.maxCacheSize}, clearing old entries...`
    );

    // Sort items by access count (LRU)
    const sortedItems = Object.entries(state.itemDetails.itemsCache).sort(
      ([, a], [, b]) => (a as CachedItemDetails).accessCount - (b as CachedItemDetails).accessCount
    );
  ...
  _fetchItemData: async (itemId: string, userId?: string) => {
    ...
    return {
      ...
      audioFiles,
      progress,
      authorId: (authors as { authorId: string }[])[0]?.authorId ?? null,
      seriesId: (seriesList as string[])[0] ?? null,
      lastUpdated: Date.now(),
      accessCount: 1,
    };
  },
