...
import { getItemsNeedingFullRefresh } from '@/db/helpers/homeScreen';
...
export class LibraryItemBatchService {
  ...
  async processBackgroundQueue(userId: string, force: boolean = false): Promise<void> {
    ...
    try {
      ...
      // First priority: items with progress that need refresh
      let itemIds = await getItemsNeedingFullRefresh(userId, this.BATCH_SIZE);
      console.log(`[LibraryItemBatchService] Found ${itemIds.length} items with progress needing refresh`);

      if (itemIds.length === 0) {
        // Second priority: items that don't have full metadata yet
        console.log('[LibraryItemBatchService] No items with progress need refresh, checking for items without metadata');
        itemIds = await getLibraryItemsNeedingFullData(this.BATCH_SIZE);
