...
must_haves:
  truths:
    - "PROP-01 verified: grep confirms zero coordinator-owned store writes in service files"
...
- Also add a comment: `// Static verification: grep -rn "store\.\(updatePosition\|updatePlayingState\|_setTrackLoading\|_setCurrentTrack\|_setPlaySessionId\|_setSeeking\|_setPlaybackRate\|_setVolume\)" src/services/PlayerService.ts src/services/PlayerBackgroundService.ts — should only return documented exceptions`
...
- Add comment: "Periodic metadata updates (2s gate) are preserved in BGS handlePlaybackProgressUpdated — not tested here as that's BGS-specific, not bridge-specific"
  ...
  <done>
  PROP-01 through PROP-06 contract tests exist. PROP-01 verified via lifecycle test. PROP-02 documented as structurally satisfied. PROP-03 tested with render count pattern or documented with manual verification. PROP-04 verified sleep timer untouched by bridge. PROP-05 verified BGS graceful failure. PROP-06 verified chapter-change metadata firing.
  </done>
  </task>
...
4. Manual grep verification matches PROP-01:
